...
: Endpoint<GetSavingsForecastRequest, GetSavingsForecastResponse>
{
    ...
    public override async Task HandleAsync(GetSavingsForecastRequest request, CancellationToken ct)
    {
        ...
        int forecastedMonths = await savingsForecastService.CalculateForecastAsync(user.Id, request.SavingId, request.MainIncomeStrategyId, request.SideIncomeStrategyId);
